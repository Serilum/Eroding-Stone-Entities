...
public class Util {
	...
	public static boolean populateArrays() {
		...
		for (String itemstring : tosandstring.split(",")) {
			ResourceLocation rl = ResourceLocation.parse(itemstring.trim().replaceAll("[^a-z0-9_.-:]", ""));
			...
			Optional<Holder.Reference<Item>> itemOptionalReference = BuiltInRegistries.ITEM.get(rl);
			itemOptionalReference.ifPresent(itemReference -> erodeinto.put(itemReference.value(), Items.SAND));
		}
		...
		for (String itemstring : toredsandstring.split(",")) {
			...
			Optional<Holder.Reference<Item>> itemOptionalReference = BuiltInRegistries.ITEM.get(rl);
			itemOptionalReference.ifPresent(itemReference -> erodeinto.put(itemReference.value(), Items.RED_SAND));
		}
		...
		for (String itemstring : toclaystring.split(",")) {
			...
			if (itemOptionalReference.isPresent()) {
				Item item = itemOptionalReference.get().value();
